...
def remove_trailing_period(value):
    if isinstance(value, str) and value.endswith('.'):
        return value[:-1]
    return value
X = pd.read_csv('E:\\My_projects\\\unified_data\\rdkit_right_desc.csv', index_col=0)
y = pd.read_csv('E:\\My_projects\\\unified_data\\rdkit_right_desc.csv', index_col=0)['0']
y = pd.DataFrame(y)
...
def remove_trailing_period(value):
    if isinstance(value, str) and value.endswith('.'):
        return value[:-1]
    return value
X = pd.read_csv('E:\\My_projects\\\unified_data\\rdkit_right_desc.csv', index_col=0)[features]
y = pd.read_csv('E:\\My_projects\\\unified_data\\rdkit_right_desc.csv', index_col=0)['0']
y = pd.DataFrame(y)
cons = pd.read_csv('E:\\My_projects\\unified_data\\unified_data\\info_right_desc.csv')['siRNA concentration'].apply(remove_trailing_period).astype(float)
X['Concentration_nM'] = cons[cons <= 100]
...
y_pred_prob_test = model.predict_proba(X_valid)[:, 1] # вероятности для ROC AUC
...
roc_auc = roc_auc_score(y_valid, y_pred_prob_test)
fpr, tpr, thresholds = roc_curve(y_valid, y_pred_prob_test)
